...
def median_filter(image, rad):
    ...
    """
    image_= np.copy(image).astype(np.float32)
    padded_image = np.pad(image_, rad, mode='edge')
    filtered_image = np.zeros_like(image_)
...
def gaussian_filter_custom(image, sigma_d):
    ...
    padded_image = np.pad(image_, radius, mode='edge')
    filtered_image = np.zeros_like(image_)
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            window = padded_image[i:i + size, j:j + size]
            filtered_image[i, j] = np.sum(window * kernel)
    filtered_image = (filtered_image.clip(0, 255)).astype(np.uint8)        
    return filtered_image
...
def psnr(image1, image2):
    ...
    """
    image_1 = np.copy(image1).astype(np.float32)
    image_2 = np.copy(image2).astype(np.float32)
    eps = 1e-11
    mse_value = max(mse(image_1, image_2), eps)
    if mse_value == 0:
        return float('inf')
    max_pixel = 255.0
    return 10 * np.log10(max_pixel**2 / mse_value, dtype=np.float32)
...
def bilateral_filter(image, size=5, sigma_d=10.0, sigma_r=25.0):
    ...
    padded_image = np.pad(image_, pad_width, mode='edge').astype(np.float32)
    h, w = image.shape
    ...
    filtered_image = np.zeros_like(image_, dtype=np.float32)
    ...
    for i in range(h):
        for j in range(w):
            ...
            filtered_image[i, j] = bilat
    filtered_image = np.ceil(filtered_image)
    filtered_image = np.clip(filtered_image, 0, 255).astype(np.uint8)
    return filtered_image
...
def main():
    ...
    if command in ['median', 'gauss', 'bilateral']:
        if command == 'median':
            ...
            image = load_image(input_file).astype(np.float32)
        ...
        image1 = load_image(input_file_1).astype(np.float32)
        if len(image1.shape) == 3:  
            image1 = image1[:, :, 0]
        image2 = load_image(input_file_2).astype(np.float32)
